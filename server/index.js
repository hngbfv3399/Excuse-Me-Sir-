...
io.on("connection", (socket) => {
  ...
  socket.on("room:start", () => {
    ...
    if (redTeam.length === 0 || blueTeam.length === 0) return socket.emit("game:alert", "양 팀에 최소 1명 이상 있어야 합니다.");
    if (redTeam.length !== blueTeam.length) return socket.emit("game:alert", "양 팀의 인원 비율이 1:1로 똑같아야 합니다.");
    
    const unreadyPlayers = playersArr.filter(([id, p]) => id !== room.hostId && !p.isReady);
    if (unreadyPlayers.length > 0) return socket.emit("game:alert", "모든 플레이어가 준비 완료를 해야 합니다.");
    ...
    io.to(roomId).emit("game:score_update", { score: room.score, maxScore: room.maxScore });
    io.to(roomId).emit("game:update", room.players);
    broadcastRooms(); 
  });
  ...
  socket.on("player:move", ({ x, y }) => {
    ...
    if (!player.isTagger && !player.isJailed) {
      ...
      if (!player.carriedItem) {
        for (let i = 0; i < room.items.length; i++) {
           ...
           const isIntersect = player.x < it.x + 20 && player.x + 20 > it.x && player.y < it.y + 20 && player.y + 20 > it.y;
           if (isIntersect) {
      ...
      if (player.carriedItem) {
        if (player.x >= 160 && player.x <= 360 && player.y >= 160 && player.y <= 360) {
          ...
          if (room.score >= room.maxScore) {
            ...
            io.to(roomId).emit("game:alert", "🏆 의적 홍길동 무리가 금괴를 모두 훔쳐 승리했습니다!");
          }
        }
      }
    }
        ...
        if (p1.isTagger && !p2.isTagger && !p2.isJailed) {
          ...
          if (isIntersect) {
            ...
            if (p2.carriedItem) {
               ...
               room.items.push({ id: `gold_${crypto.randomUUID()}`, x: p2.x, y: p2.y });
               io.to(roomId).emit("game:items_update", room.items);
            }
            // 감옥(관아) 중앙 (타일 20 -> 800)
            p2.x = 800;
    ...
    if (caughtPlayerId) {
      ...
      if (allRunners.length > 0 && allJailed) {
        room.gamePhase = "ended";
        io.to(roomId).emit("game:phase_change", { gamePhase: room.gamePhase, timeLeft: 0 });
        io.to(roomId).emit("game:alert", "🔥 탐관오리(술래)가 의적을 전원 감옥에 가뒀습니다!");
      }
  ...
  socket.on("rescue:start", () => {
    ...
    for (const id in room.players) {
      if (room.players[id].isTagger) {
        io.to(id).emit("game:alert", "🚨 누군가 감옥의 문을 해킹하고 있습니다! (방어 필요)");
      }
    }
  });
  ...
  socket.on("rescue:complete", () => {
    ...
    if (rescued > 0) {
      io.to(roomId).emit("game:alert", "🔓 감옥 문이 파괴되어 동료들이 탈출해 안전구역으로 향했습니다!");
      io.to(roomId).emit("game:reset", room.players);
    }
  });
...
server.listen(3001, () => {
  console.log("Server running on port 3001 (Serving React App & Scalable Sockets)");
});
